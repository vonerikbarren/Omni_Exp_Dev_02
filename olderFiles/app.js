        ...
        function setupMySpaceGrid() {
            ...
            // Current spherical grid implementation (existing code)
            ...
            for (let i = 0; i <= divisions; i++) {
                ...
                const material = new THREE.LineBasicMaterial({ color: 0x808080, opacity: 0.3, transparent: true });
        ...
        function onMouseDown(event) {
            if (event.button === 0) {
                ...
                if (intersects.length > 0) {
                    ...
                    clickedObj.material.color.setHex(0x808080);
        ...
        function updateInspectorPanel() {
            if (selectedObject) {
                ...
                if (savedData) {
                    ...
                    // Update label if header exists
                    if (data.header) {
                        updateObjectLabel(selectedObject, data.header);
                    }
                } else {
        ...
        function setupInspectorDataSave() {
            ...
            submitBtn.addEventListener('click', () => {
                if (selectedObject) {
                    ...
                    // Create or update label for the object
                    updateObjectLabel(selectedObject, header);
        ...
        function updateObjectLabel(object, headerText) {
            if (!object || !headerText) {
                // Remove label if header is empty
                if (object.userData.label) {
                    object.remove(object.userData.label);
                    if (object.userData.label.geometry) object.userData.label.geometry.dispose();
                    if (object.userData.label.material) object.userData.label.material.dispose();
